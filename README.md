...
```
* Pytorch is optional if users only need the provided XGBoost model. It's required to build DNN models.
...
## Use examples
* A XGBoost model, which was the best one as shown in our [paper], is provided in `./Models/xgboost_rdkit_2d_clustering_model.job`. It can be used directly for prediction.
...
* Put those two joblib directly under `./Datasets`. To view them, please refer to this notebook: `./Jupyter/data_overview.ipynb`.
